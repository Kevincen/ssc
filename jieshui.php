...
/* 命令行执行代码，php jieshui.php "expect=xx&opencode=yy" */
if (isset($argc)) {
	if ($argc > 0) {
		for ($i = 1; $i < $argc; $i++) {
	...
	case "gdkl10f" :
		{
			...
			$List['openTerm'] = $_REQUEST['expect']; // 开奖期数
			$List['openResult'] = explode(",", $_REQUEST['opencode']); // 开奖结果，哪些球，逗号隔开的字符串，改为数组
			$List['enddt'] = isset($_REQUEST['enddt']) ? $_REQUEST['enddt'] : "";
			// 2：下期开盘，3：正在开盘，1：未开盘
			...
			if ($result && $result[0]['g_qishu'] <= $List['openTerm']) {
				...
				$lastNum = mb_substr($number, - 2); // 获取倒数的两个数字，期数格式为YYYYMMDD[qishu]
				if ($lastNum < 84) {
					$nextNumber = $number + 1;
					// 刪除開獎時間已經結束的期數、最後一期不執行刪除
				...
				// 如果没有在开奖结果里面查到新的一期开盘结果，说明是刚开的，需要插入history
...
function getNumberByBall_gd($_number) {
	...
	// 還原賠率，这个坑爹的设定，把default中的东西插入到g_odds中
	initializeOdds();
	// 降賠率
	global $ConfigModel;
	// 自动降赔率，这个没用到，数据库里面是0
