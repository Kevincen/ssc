...
class SumAmountnc
{
    ...
    private function ResultCorrespond($numberList, $resultList, $param = 0)
    {
        if ($param == 0 && Copyright) {
            ...
            $sum_LM = new sumAmount('123');//借用一下，所以瞎写了一个期数 by 2b
...
//				
                ...
                $resultList[0] = $result[0];
                $resultList[1] = $result[1];
            } else if ($resultList['g_mingxi_1'] == '任选二' && Copyright) {
                    // 任選二規則、8個中2個 視為中獎
                    ...
                    $param = $sum_LM->SumLM1($_numberList, $resultList, $index);
                    $resultList[0] = $param;
                    $resultList[1] = 'LM';
                } else if ($resultList['g_mingxi_1'] == '任选三' && Copyright) {
                    // 任選二規則、8個中3個 視為中獎
                    $result = $sum_LM->SumLM($numberList, $resultList, 3);
                    $resultList[0] = $result[0];
                    $resultList[1] = $result[1];
                }else if ($resultList['g_mingxi_1'] == '选二连直')
                 {
                     //todo:找2a一起来解决
                 //選二連直規則 任意三個號碼相連、並且對應下注號碼的前後。視為中獎
                ...
                else if ($resultList['g_mingxi_1'] == '选三前组' && Copyright) {
                    ...
                    $resultList[0] = $param;
                    $resultList[1] = 'LM';
                } else if ($resultList['g_mingxi_1'] == '任选四' && Copyright) {
                    // 任選四規則、8個中4個 視為中獎
                    $result = $sum_LM->SumLM($numberList, $resultList, 4);
                    $resultList[0] = $result[0];
                    $resultList[1] = $result[1];
                } else if ($resultList['g_mingxi_1'] == '任选五' && Copyright) {
                    // 任選五規則、8個中5個 視為中獎
                    $result = $sum_LM->SumLM($numberList, $resultList, 5);
                    $resultList[0] = $result[0];
                    $resultList[1] = $result[1];
            } else {
